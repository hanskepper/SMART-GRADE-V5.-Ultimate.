...
(function() {
  ...
  function checkForUpdates() {
    ...
    fetch('./version.json?t=' + Date.now(), { cache: 'no-store' })
      ...
      .then(function(data) {
        var remoteVersion = data.version || data.versionCode || '0';
        var localVersion = localStorage.getItem('smartgrade_app_version') || APP_VERSION;
        
        if (remoteVersion !== localVersion && remoteVersion !== '0') {
          console.log('[Update] New version available: ' + remoteVersion);
          localStorage.setItem('smartgrade_app_version', remoteVersion);
          
          // Silent update - reload after 2 seconds
          setTimeout(function() {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', startUpdateMonitoring);
  } else {
    startUpdateMonitoring();
  }
  
  console.log('[Update] Silent auto-update started - v' + APP_VERSION);
